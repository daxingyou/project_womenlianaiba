	...
	function createMissingUnity(id, attributes, params, callback, broken) {
		...
		if (re) {
			...
			if (ua.win && ua.ie) {
				...
				var exefile=ua.x64?"UnityWebPlayerFull64.exe":"UnityWebPlayerFull.exe";
				at="<a id=setup href="+ exefile +" target=_blank></a><span id=arrow></span>";
				if (fullSizeMissing) {
					//var ft = '<div style="width: ' + appendPX(imgWidth) + '; margin: auto; position: relative; top: 50%;">' + at + '</div>';
					var ft = '<div style="width: 760px; height:600px; text-align:left; margin: auto; position: relative;  background-image:url(../images/plugin/bg.png);">' + at + '</div>';
				...
				if (fullSizeMissing) {
					...
					fe.style.backgroundImage="url(../images/plugin/bg.png)";
					//fe.style.top = "50%";
				}
				var ae = doc.createElement("a");
				ae.setAttribute("target", "_blank");
				ae.setAttribute("href", "UnityWebPlayerFull.exe");
