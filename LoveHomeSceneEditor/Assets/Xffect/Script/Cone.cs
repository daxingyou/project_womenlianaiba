    ...
    public class Cone
    {
        ...
        protected void UpdateRotAngle(float deltaTime)
        {
            ...
            for (int i = NumVerts / 2; i < NumVerts; i++)
            {
                ...
                Verts[i] = Vector3.RotateTowards(Verts[i], Verts[i - NumVerts / 2], SpreadAngle * Mathf.Deg2Rad, 1);
            }
        }
        ...
        public void UpdateVerts()
        {
            ...
            {
                // ... don't use Quanternion.EulerAngles()... shit.
        ...
        {
            if (Owner.Owner.RotAffectorEnable)
                UpdateVerts();

            for (int i = 0; i < NumSegment + 1; i++)
            {
                VertsTemp[i] = Verts[i] * Scale.x + Position;
            }
