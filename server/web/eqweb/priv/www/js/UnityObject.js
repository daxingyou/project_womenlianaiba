...
var unityObject = function() {
	...
	baseUrl = "http://app35398.imgcache.qzoneapp.com/app35398/WebPlayer/",
	fullInstall = true,
	autoInstall = false,
	enableJava = true,
	...
	function createMissingUnity(id, attributes, params, callback, broken, prevType, lastError) {
		...
		if (re) {
			...
			if (broken) {
				...
				var msg = "开始游戏!";
				var img = "../images/setup_normal.png";
				var imgWidth = 406;
				var imgHeight = 110;
			}
			...
			if (ua.win && ua.ie) {
				...
				if (fullSizeMissing) {
					var ft = '<div style="width: ' + appendPX(imgWidth) + '; margin: auto; position: relative; top: 490px;">' + at + '</div>';
					re.outerHTML = '<div id="clickoncebg"><div id="' + id + '" style="width: ' + appendPX(width) + '; height: ' + appendPX(height) + '; text-align: center;' + st + '">' + ft + '</div></div>';
				}
				else {
	...
	return {
	    getVersion : function(callback){
	        if (unityObject.ua.win && unityObject.ua.ie) 
	        {
	            var uo = new ActiveXObject("UnityWebPlayer.UnityWebPlayer.1");
	            var v = {};
				v.plugin = uo.GetPluginVersion();
				callback(v);
			}
			else
			{
	            getPluginVersion(callback);
	        }
	    },
	    getNumberVersion : function(version)
	    {
	        return getNumericUnityVersion(version);
	    },
